...
// level 500 display tv shows UI elements
// const mainTvShowData = {
//   name:"",
//   image: "",
//   summary: " ",
...
const addDropDownMenu = (episodeList) => {
  ...
  select.addEventListener("change", function (event) {
    ...
    let episodeContainers = document.querySelectorAll(".episode-container-class-name"); // returns one node 
    // console.log(episodeContainers)
    // let episodeContainers = document.getElementsByClassName("episode-container-class-name"); // An array of episode container returns html selector
    ...
    if (event.target.value !== "") {
      for (let index = 0; index < episodeContainers.length; index++) {
        const episodeContainer = episodeContainers[index];
          //title.className = "title";

        if (episodeContainer.querySelector(".title").textContent === event.target.children) {
    ...
    // if (event.target.options[event.target.selectedIndex].index === 0) { // selectorIndex gives you the index of the selected option
    //   // refers to the all episode option

    //   episodeList.forEach((episodeOption) => { // 
    //     episodeCounter = episodeList.length;
    //     if (episodeContainers[episodeIndex]) {
    //       episodeContainers[episodeIndex].style.display = "";
    //       episodeIndex++; // This increments});
    //     }
    //   });
    ...
    //   episodeList.forEach(() => {
    //     if (
    //       episodeIndex === event.target.options[event.target.selectedIndex].index - 1 // test if the current index is equal to the index
    //     ) {
    //       showEpisode(episodeContainers[episodeIndex]); //.style.display = "";
    //     } else {
    //       hideEpisode(episodeContainers[episodeIndex]); //.style.display = "none";
    ...
    //   episodeCounter = 1;
    // }
    const allEpisodes = document.querySelectorAll(
      "episode-container-class-name"
    );
    // const visibleEpisodes = [...allEpisodes].filter((container) => {
    ...
    updateCounter(episodeCounter, allEpisodes.length);
...
function updateCounter(numberOfEpisodes, totalEpisodes) {
  // const allEpisodes = document.querySelectorAll("episode-container-class-name");
  // const visibleEpisodes = [...allEpisodes].filter(container => {
  //   container.style.display !== "none";  
  // });
  ...
  counterDisplay.innerText = `Displaying ${numberOfEpisodes.length} / ${totalEpisodes.length} episodes`; // update of the counter
  //episodeContainers.filter((container) => container.style.display !== "none").length;
...
informationMissing.id = "informationMissing";
errorContainer.appendChild(informationMissing); 
  ...
  inputBox.addEventListener("keyup", (event) => {
    ...
    // informationMissing.setAttribute("id", "informationMissing");
    ...
    episodes.forEach((episode) => {
      ...
      //   // informationMissing.innerText = "Sorry, the keyword does not exist in this database"
      //   // errorContainer.appendChild(informationMissing)
      //   //console.log(informationMissing.innerText)
      // }
      else {
