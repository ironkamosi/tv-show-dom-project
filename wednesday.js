...
// level 500 List all tv shows
// api fetch for level 500 
function getAllTvShowsApi() {
  const allTvShowData = getAllShows().sort(compare);
  allTvShowData.forEach(element => {
    allTvShowsListings(element)
    console.log(element)
  });
...
function allTvShowsListings(tvShow) {
  ...
  const allTVShowsContainer = document.createElement("div");
  allTVShowsContainer.setAttribute("id", "allTvShowsContainer");
  ...
  let tvShowsInformationContainer = document.createElement("div");
  let tvShowsInformation = document.createElement("ul");
  ...
  allTVShowsContainer.appendChild(tvShowsInformationContainer);
  mainDiv.appendChild(allTVShowsContainer)
  
  mainTvInfo.appendChild(tvShowsTitle)
  mainTvInfo.appendChild(tvShowsImage);
  mainTvInfo.appendChild(tvShowsSummary);

  tvShowsInformationContainer.appendChild(mainTvInfo);
  tvShowsInformationContainer.appendChild(tvShowsInformation);
  tvShowsInformation.appendChild(tvShowsRating);
  tvShowsInformation.appendChild(tvShowsRunTime);
  tvShowsInformation.appendChild(tvShowsStatus);
  tvShowsInformation.appendChild(tvShowsGenre);

  console.log(allTVShowsContainer)
}
...
const addDropDownMenu = (episodeList) => { // select on the right for episodes 
  let option;
  let select = document.getElementById("select");
...
"episode-container-class-name"
    ...
    if (event.target.options[event.target.selectedIndex].index === 0) {
      ...
      episodeList.forEach(() => {
        ...
        episodeCounter = episodeList.length;
        episodeContainers[episodeIndex].style.display = "block";
        episodeIndex++; // This increments});
      });
    } else {
      ...
      // console.log(`episode containers${episodeContainers}`);
        ...
        if (
          ...
          // console.log(`hideEpisode in else statement ${episodeIndex}`);
...
; // when the browser is load call set up
