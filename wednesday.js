...
function allTvShowsListings(tvShow) {
  ...
  tvShowsInfoContainer.setAttribute("id", "infoContainer");
  ...
  tvShowsTitle.addEventListener("click", event => {  // this hides the tv show when the tv show is selected the episodes appear 
    let showId = event.target.getAttribute("showId");
    console.log(showId)
  //   Array.from(tvShowsInfoContainer).forEach((element) => {
...
function getAllEpisodesApi(id) {
  ...
  // let arrayOfEpisodes;
  console.log("inside getAllEpisodesApi")
  fetch(`https://api.tvmaze.com/shows/${id}/episodes`)
...
/**
 ...
 4- create a button to redisplay the shows 
 5- add a 
 */