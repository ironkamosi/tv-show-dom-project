...
// set it to be the first show on the select
...
function removeEpisodes(episodeList) { 
  ...
  episodeList.forEach(element => { // loops over the array and removes each episode from the dom when the eventListener is activated 
...
function addShowSelector(showData) { // the selector on the left
  ...
  showData.forEach((show) => {
   ...
   // context whats in context right now
  select.addEventListener("change", function (event) {
    const showFilter = getAllShows().filter(element => { // filters show list
      return element.name === event.target.value
    });
    // element.id
    getAllEpisodesApi(showFilter[0].id).then(data => {
      showContext = {
        showName :event.target.value,
        showId : showFilter[0].id,
        showEpisodes : data,
      }
...
const addSearchBox = (episodeList) => {
  ...
  inputBox.addEventListener("keyup", (event) => {
    ...
    let queryResult = showContext.showEpisodes.filter(episode => {
      return (
        episode.name.toLowerCase().includes(searchString.toLowerCase()) ||
        episode.summary.toLowerCase().includes(searchString.toLowerCase())
      );
      })
    // console.log(queryResult)
    // showContext.showEpisodes.forEach((episode) => { // global variable // episode list
    //   if (
...
function getAllEpisodesApi(id) { // function to get the apis 
  ...
  .then((response) => response.json().then((data) => data))
  .then((allEpisodes) =>  allEpisodes) // gets the episode data and sets it to an array of episodes
  .catch((error) => console.log(error));
}
window.onload = setup;
