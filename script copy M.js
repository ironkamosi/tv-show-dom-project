  ...
  inputBox.addEventListener("keyup", (event) => {
    ...
    episodes.forEach((episode) => {
      ...
      if (
        ...
        informationMissing.innerText =
          "Sorry, the keyword does not exist in database";
        errorContainer.appendChild(informationMissing);
        //console.log(informationMissing.innerText)
      } else {
        hideEpisode(episodeContainers[episodeIndex]);
...
function makePageForEpisodes(shows) {
  ...
  fetch(`https://api.tvmaze.com/shows/${showId}/episodes`)
    .then((response) => response.json()) // remove the end of the json once bug is fixed with the header// return response.json this gathers the data
    .then((allEpisodes) => {
      episodes = allEpisodes;
      addHeader(episodes, showsList);
      addAllEpisodes(allEpisodes);
      addFooter();
    }) // this does nt get called until we get the data
    .catch((error) => console.log(error));
